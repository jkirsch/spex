...
import edu.tuberlin.spex.matrix.memory.UnsafeMemory;
...
public class AdaptedCompRowMatrixUnsafe extends AbstractMatrix implements Value {
     ...
     *
     * @param r Reader to get sparse matrix from
     */
    public AdaptedCompRowMatrixUnsafe(MatrixVectorReader r) throws IOException {
     ...
     * @param nz         The nonzero column indices on each row
     */
    public AdaptedCompRowMatrixUnsafe(int numRows, int numColumns, int[][] nz) {
     ...
     */
    public AdaptedCompRowMatrixUnsafe(int numRows, int numColumns, int[][] nz, double[] data) {
        super(numRows, numColumns);
        construct(nz);
        this.data = data;
    }
    ...
    private void construct(Matrix A, boolean deep) {
        if (deep) {
            if (A instanceof AdaptedCompRowMatrixUnsafe) {
                AdaptedCompRowMatrixUnsafe Ac = (AdaptedCompRowMatrixUnsafe) A;
            ...
            no.uib.cipr.matrix.sparse.CompRowMatrix Ac = (no.uib.cipr.matrix.sparse.CompRowMatrix) A;
     ...
     * Constructor for CompRowMatrix
     *
     * @param A    Copies from this matrix
     * @param deep True if the copy is to be deep. If it is a shallow copy,
     *             <code>A</code> must be a <code>CompRowMatrix</code>
     */
    public AdaptedCompRowMatrixUnsafe(Matrix A, boolean deep) {
        super(A);
        construct(A, deep);
    }
     ...
     * Constructor for CompRowMatrix
     *
     * @param A Copies from this matrix. The copy will be deep
     */
    public AdaptedCompRowMatrixUnsafe(Matrix A) {
        this(A, true);
    }
    ...
    private int getIndex(int row, int column) {
        int i = Arrays.binarySearch(columnIndex,
                column, rowPointer[row], rowPointer[row + 1]);
    ...
    @Override
    public AdaptedCompRowMatrixUnsafe copy() {
        return new AdaptedCompRowMatrixUnsafe(this);
    }

    @Override
    public Iterator<MatrixEntry> iterator() {
        return new AdaptedCompRowMatrixUnsafe.CompRowMatrixIterator();
    }

    @Override
    public AdaptedCompRowMatrixUnsafe zero() {
        Arrays.fill(data, 0);
        return this;
    }
    ...
    public Matrix set(Matrix B) {
        if (!(B instanceof AdaptedCompRowMatrixUnsafe))
            return super.set(B);

        checkSize(B);

        AdaptedCompRowMatrixUnsafe Bc = (AdaptedCompRowMatrixUnsafe) B;
    ...
    private class CompRowMatrixIterator implements Iterator<MatrixEntry> {
        ...
        private AdaptedCompRowMatrixUnsafe.CompRowMatrixEntry entry = new AdaptedCompRowMatrixUnsafe.CompRowMatrixEntry();
    ...
    public static AdaptedCompRowMatrixUnsafe buildFromSortedIterator(Iterator<Tuple3<Integer, Integer, Double>> values, int rows, int cols) {
        ...
        AdaptedCompRowMatrixUnsafe constructed = new AdaptedCompRowMatrixUnsafe(rows, cols, nnz, ArrayUtils.toPrimitive(data.toArray(new Double[data.size()])));
    ...
    public void write(DataOutputView out) throws IOException {
        ...
        int byteSize = columnIndex.length + rowPointer.length + data.length * 2;
        byteSize += 3 * UnsafeMemory.SIZE_OF_INT;
        ...
        UnsafeMemory unsafeMemory = new UnsafeMemory(buffer);

        unsafeMemory.putIntArray(columnIndex);
        unsafeMemory.putIntArray(rowPointer);
        unsafeMemory.putDoubleArray(data);
    ...
    public void read(DataInputView in) throws IOException {
        ...
        columnIndex = unsafeMemory.getIntArray();
        rowPointer = unsafeMemory.getIntArray();
        data = unsafeMemory.getDoubleArray();
